(async function () {
    ...
    const userConfig = await get_user_config();
    const VERIFIED_COLOR = userConfig['verified-sig-color'];
    const UNVERIFIED_COLOR = userConfig['unverified-sig-color'];
    const MIXED_COLOR = userConfig['mixed-sigs-color'];
    const UNSIGNED_COLOR = userConfig['unsigned-msg-color'];

    const DISPLAY_MODE = parseInt(userConfig['uid-format']);
    const DISPLAY_KEYID = userConfig['display-keyids'];

    const UNKNOWN_USERID_PLACEHOLDER = userConfig['uid-placeholder'];
    ...
    function format_signed_data (data, pubring, successes, failures) {
        ...
        if (failures.length > 0)
            data += DISPLAY_KEYID ? ' [Unverified: '+failures.join(', ')+']'
                                  : ' [Unverified]';
        return data;
    }
